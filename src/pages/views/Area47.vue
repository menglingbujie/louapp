<script setup>
import loumap from "@/modal/area47"
import {countLou} from "@/utils/index"
import { useHouseInfo } from '@/utils/use/useHouseInfo';
const { info, getHouseInfo } = useHouseInfo();
const land = '47'; //地块

let loumapother = loumap.slice(0,loumap.length-2);
loumapother.push(loumap[loumap.length-1]);
const lastlou = [loumap[loumap.length-2]];
...
<div class="area a15">
  ...
  <div class="parts one">
    <div class="part" :class="'p'+index" v-for="(p,index) in lastlou" :key="'part-'+index">
      <h3 class="tpart">#{{p.part}}号楼</h3>
      <div class="list">
        <div class="unit" v-for="u,idx in p.units.reverse()" :key="'unit-'+idx">
          <h4 class="tunit">{{p.units.length-idx}}单元</h4>
