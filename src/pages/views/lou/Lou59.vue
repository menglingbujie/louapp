<script setup>
import {ref} from "vue";
import gloumap from "@/modal/area59"
...
const firstlout = gloumap.slice(0,2);
const loutother = gloumap.slice(2,gloumap.length-2);
const lastlout = gloumap.slice(gloumap.length-2);
...
<div class="area a15">
  ...
  <div class="parts double">
    <div class="part" v-for="(p,index) in firstlout" :class="'p'+index"  :key="'part-'+index">
  ...
  <div class="parts">
    <div class="part" v-for="(p,index) in loutother" :class="'p'+index"  :key="'part-'+index">
      <h3 class="tpart">#{{p.part}}号楼</h3>
      <div class="list">
        <div class="unit" v-for="u,idx in p.units.reverse()" :key="'unit-'+idx">
          <h4 class="tunit">{{p.units.length-idx}}单元</h4>
